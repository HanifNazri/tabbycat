...
class BasePublicNewBallotSetView(PersonalizablePublicTournamentPageMixin, RoundMixin, BaseBallotSetView):
    ...
    def postprocess_result(self):
        if self.ballotsub.single_adj and self.tournament.pref('disable_ballot_confirms'):
            merged_bs = BallotSubmission(debate=self.debate, submitter=None, submitter_type=BallotSubmission.Submitter.AUTOMATION, ip_address=get_ip_address(self.request), confirmed=True)
            ...
            if len(bses) != DebateAdjudicator.objects.filter(debate=self.debate).exclude(position=DebateAdjudicator.TYPE_TRAINEE).count():
                return
